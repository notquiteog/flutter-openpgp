...
function failPending(error) {
  for (const id of pending.keys()) {
    postMessage({ id, response: null, error });
  }
  pending.clear();
}
...
function scheduleReload(loadFn) {
  if (_reloadAttempts >= _maxReloadAttempts) {
    ...
    failPending("WASM failed to load");
    return;
  }
...
onmessage = async ({ data }) => {
  ...
  const payload = request || new Uint8Array();
  pending.set(id, true);
  ...
  try {
    openPGPBridgeCall(name, payload, payload.length, (error, response) => {
      pending.delete(id);
    ...
    self.loaded = false;
    pending.delete(id);
    postMessage({ id, response: null, error: errorMessage(e) });
  }
};
